...
'''in the process of changing all the points to the point functions, and then trying to make the grid cell size smaller, and also trying to make it dependent on the x and y lengths along and not on inputting numbers'''
'''main things to work on:
1. Saving data from each run - save x, y, z, r and time of each nuclei - should be all you need - maybe save each plot?
...
#n=1.7
    print(t)    
    SA = NUCLEI[:,3]*NUCLEI[:,3] * np.pi*4/1E6/1e6/2  #surface area of each hemisphere in m2  
    Delta_moles = Growth_Rate*SA*delta_t #mol per sphere
    Delta_volume = Delta_moles*molarV_arag
    VOL = (NUCLEI[:,3]*NUCLEI[:,3]*NUCLEI[:,3] * np.pi*4/3)/2 #volume of each hemisphere in m2
...
#        NUCLEI_3ROW[row,2] = newz3
    ...
    def Discrete3dSurface():
        ...
        for xx in np.arange(0,x_length+.5,0.5):
            
            for yy in np.arange(0,y_length+.5,0.5):
    ...
    def areaofIrregularQuad(l1,l2,l3,l4):
        ...
        diagonal_len = distanceFormula(point1ofEachCell()[0],point1ofEachCell()[1],point1ofEachCell()[2],point3ofEachCell()[0],point3ofEachCell()[1],point3ofEachCell()[2])
    ...
    def point1ofEachCell():
        '''defining the points of each cell in the grid so that they can be used later for determining area, volume, etc'''
        for ones in np.arange(0,y_length*2):
            grid_range = np.array(np.arange(ones*x_length*2+ones,x_length*2 + ones*x_length*2+ones))
            for point in grid_range:
                p1x = surface_points[point,0]
                p1y = surface_points[point,1]
                p1z = surface_points[point,2]
        return p1x, p1y, p1z   
     
    def point2ofEachCell():
        '''defining the points of each cell in the grid so that they can be used later for determining area, volume, etc'''
        for ones in np.arange(0,y_length*2):
            grid_range = np.array(np.arange(ones*x_length*2+ones,x_length*2 + ones*x_length*2+ones))
            for point in grid_range:
                p2x = surface_points[point+(2*x_length+1),0] 
                p2y = surface_points[point+(2*x_length+1),1] 
                p2z = surface_points[point+(2*x_length+1),2]
        return p2x, p2y, p2z 
        
    def point3ofEachCell():
        '''defining the points of each cell in the grid so that they can be used later for determining area, volume, etc'''
        for ones in np.arange(0,y_length*2):
            grid_range = np.array(np.arange(ones*x_length*2+ones,x_length*2 + ones*x_length*2+ones))
            for point in grid_range:
                p3x = surface_points[point+(2*x_length+2),0]
                p3y = surface_points[point+(2*x_length+2),1]
                p3z = surface_points[point+(2*x_length+2),2]
        return p3x, p3y, p3z
        
    def point4ofEachCell():
        '''defining the points of each cell in the grid so that they can be used later for determining area, volume, etc'''
        for ones in np.arange(0,y_length*2):
            grid_range = np.array(np.arange(ones*x_length*2+ones,x_length*2 + ones*x_length*2+ones))
            for point in grid_range:
                p4x = surface_points[point+1,0] 
                p4y = surface_points[point+1,1] 
                p4z = surface_points[point+1,2]           
        return p4x, p4y, p4z 
    ...
    for ones in np.arange(0,y_length*2):
        ...
        for point in grid_range:
            l1 = distanceFormula(point1ofEachCell()[0],point1ofEachCell()[1],point1ofEachCell()[2],point2ofEachCell()[0],point2ofEachCell()[1],point2ofEachCell()[2])
            l2 = distanceFormula(point2ofEachCell()[0],point2ofEachCell()[1],point2ofEachCell()[2], point3ofEachCell()[0],point3ofEachCell()[1],point3ofEachCell()[2])
            l3 = distanceFormula(point3ofEachCell()[0],point3ofEachCell()[1],point3ofEachCell()[2],point4ofEachCell()[0],point4ofEachCell()[1],point4ofEachCell()[2])
            l4 = distanceFormula(point3ofEachCell()[0],point3ofEachCell()[1],point3ofEachCell()[2], point1ofEachCell()[0],point1ofEachCell()[1],point1ofEachCell()[2])            
            #something is up with this - keeps producing nans            
            area_cell = areaofIrregularQuad(l1,l2,l3,l4,point1ofEachCell()[0],point1ofEachCell()[1],point1ofEachCell()[2],point3ofEachCell()[0],point3ofEachCell()[1],point3ofEachCell()[2])
    ...
    point1_y = (area_count%(x_length*2))/2
    point1_x = (area_count - point1_y)/(x_length*2)
    ...
    #I divided the surface grid into 0.5x0.5 size boxes to make them smaller, so have to make sure the next point is generated in the correct box
    new_weighted_x = random.random()-0.5 + point1_x
    new_weighted_y = random.random()-0.5 + point1_y    
...
#seems reasonable
    ...
    if Nuclei_flag == 1:  #added the =1 part, assume this is what was meant, because if flag = 0, then an error occurs and don't add the nuclei, but if it's 1, then it's ok to add
     ...
     ##function for the total growth of the entire structure of the coral skeleton
#    def totalGrowth():
#        '''we know the number of nuclei total, and we divide that by the total volume of the structure
#        we can get the total volme of the structure by multiplying the calculated areas of each box in the grid 
#        across the structure by the height of the height of the point1 vertice - definitely is assuming height is
#        consistent across the box which isn't true - think about this assumption more'''
#        point1_list = [[-100,-100,-100]]        
#        for ones in np.arange(0,y_length*2):
#       
#            grid_range = np.array(np.arange(ones*200+ones,x_length*2 + ones*200+ones))
#    
#            for point in grid_range:
#                point1 = [surface_points[point,0],surface_points[point,1],surface_points[point,2]]
#                point1_list = np.append(point1_list,point1, axis=0)
#        height_list = point1_list[:,2]
#        height_list = np.delete(height_list,0,0)
#        volume_cells = height_list * all_areas
#        total_volume = sum(volume_cells)
#        nuclei_density = len(NUCLEI[:,0])/total_volume
#        return total_volume, nuclei_density
    
#    def totalGrowthAnotherWay():
#        '''totalGrowth function defines growth by using point1 of each box as the height - not exactly accurate 
#        want to test measuring growth another way by averaging the four points of the box and using that as the height'''
#        point1_list = np.array([[-100,-100,-100]])
#        point2_list = np.array([[-100,-100,-100]]) 
#        point3_list = np.array([[-100,-100,-100]])
#        point4_list = np.array([[-100,-100,-100]])
#        for ones in np.arange(0,y_length*2):
#       
#            grid_range = np.array(np.arange(ones*200+ones,x_length*2 + ones*200+ones))
#    
...
#                point3 = [surface_points[point+202,0],surface_points[point+202,1],surface_points[point+202,2]]
#                point4 = [surface_points[point+201,0], surface_points[point+201,1], surface_points[point+201,2]]       
#                point1_list = np.append(point1_list,point1, axis=0)
#                point2_list = np.append(point2_list,point2, axis=0)
#                point3_list = np.append(point3_list,point3, axis=0)
#                point4_list = np.append(point4_list,point4, axis=0)
#        height1_list = point1_list[:,2]
#        height1_list = np.delete(height1_list,0,0)
#        height2_list = point2_list[:,2]
#        height2_list = np.delete(height2_list,0,0)
#        height3_list = point3_list[:,2]
#        height3_list = np.delete(height3_list,0,0)
#        height4_list = point4_list[:,2]
#        height4_list = np.delete(height4_list,0,0)
#        ave_heights = []
#        list_len = np.size(height1_list)
#        for index in range(list_len):
#            average = (height1_list[index]+height2_list[0]+height3_list[0]+height4_list[0])/4
#            ave_heights.append(average)
#        volume_cells = ave_heights * all_areas
#        total_volume = sum(volume_cells)
#        actual_volume = sum(VOL)
#        return total_volume, actual_volume       
...
#point3 = [[1,1,3]]
    def totalVolume():
        ...
        for ones in np.arange(0,y_length*2):
            ...
            for point in grid_range:
                point1 = [surface_points[point,0],surface_points[point,1],surface_points[point,2]]
                point2 = [surface_points[point+201,0], surface_points[point+201,1], surface_points[point+201,2]] 
                point3 = [surface_points[point+202,0],surface_points[point+202,1],surface_points[point+202,2]]
                point4 = [surface_points[point+1,0], surface_points[point+1,1], surface_points[point+1,2]]
...
#                v213 = point2[0]*point2[1]*point3[2]
#                v123 = point1[0]*point2[1]*point3[2]
#                
#                vol_tri1 = (1/6)*(-v321 + v231 + v312 -v132 - v213 + v123)
#                
                tri1 = np.append(point1, point2, axis=0)
                tri1 = np.array(np.append(tri1, point3, axis=0))
                vol_tri1 = (1/6)*np.linalg.det(tri1)
                #print(vol_tri1)
#                v431 = point4[0]*point3[1]*point1[2]
#                v341 = point3[0]*point4[1]*point1[2]
#                v413 = point4[0]*point1[1]*point3[2]
#                v143 = point1[0]*point4[1]*point3[2]
#                v314 = point3[0]*point3[1]*point4[2]
#                v134 = point1[0]*point3[1]*point4[2]
#                vol_tri2 = (1/6)*(-v431 + v341 + v413 -v143 - v314 + v134)
...
#
    ...
    if t == max_t-delta_t:                                                  #save the last timestep image, and this also keeps that plot open
        fig.savefig('/Users/Marta/Documents/Python/Plots/modelnucleisphere_om25.png')
...
###plotting the surface of the nuclei   
    ...
    plt.pause(.001)    
    if t == max_t-delta_t:                                                  #save the last timestep image, and this also keeps that plot open
        fig.savefig('/Users/Marta/Documents/Python/Plots/modelnucleisurface_om25.png')
...
#growth_output = totalGrowth()
#print('Total growth:', growth_output[0])
#print('Nuclei to Total Growth Ratio:', growth_output[1])
#print('Nuclei density:', nucleiDensity())
#growth_output2 = totalGrowthAnotherWay()
#print('Total growth another way:', growth_output2[0])
#print('Volume of all Nuclei:', growth_output2[1])
